...
Eigen::MatrixXd kalman::update(Eigen::MatrixXd U_K,Eigen::MatrixXd Z_K)
{
    //先根据X_K计算X_K+1的先验
    Eigen::MatrixXd X_K_PLUS_FRONT = A * X_K + B * U_K;
    //接着计算P_K+1的先验
    ...
    //然后更新P_K
    P_K = (Eigen::MatrixXd::Identity(P_K_PLUS_FRONT.rows(),
    P_K_PLUS_FRONT.cols()) - K * H) * P_K_PLUS_FRONT;
    return X_K;
}
